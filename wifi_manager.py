...
class WifiManager:
    ...
    def __init__(self, ssid = "D-BOSS-WIFI", password = "123456789", reboot = True, debug = False):
    ...
    def handle_root(self):
        ...
        self.client.sendall("""
            ...
            <html lang="en" style="font-family: Arial, Helvetica, sans-serif;display: inline-block;text-align: center;">
                ...
                <form action="/configure" method="POST" accept-charset="utf-8">
                    <div class="topnav">
                    <h1 style="background-color: #0A1128;font-size: 1.6rem;color: white;padding: 2px;margin: 0px;">WiFi Manager</h1>
                    </div>
                    <div class="content">
        ...
        self.client.sendall("""
                                ...
                                <div class="row" style="display: flex;height: 60px;">
                                    ...
                                    <div class="col-70" style="display: inline-block;width: 70%;align-self: center;">
                                        <input type="text" placeholder="รหัสผ่าน"  id="password" name="password" style="width: 90%;padding: 12px 20px;margin: 18px;display: inline-block;border: 1px solid #ccc;border-radius: 4px;box-sizing: border-box;font-size: 1rem;">
                                    </div>
                                </div>
                                <div class="">
        ...
        if match:
            ...
            if len(ssid) == 0:
                ...
                self.send_response("""
                    ...
                    <h1>{0}</h1>
                    <p>IP address: {1}:5000</p>
                """.format(ssid, self.wlan_sta.ifconfig()[0]))
                profiles = self.read_credentials()
                profiles[ssid] = password
                self.write_credentials(profiles)
                #data = {"ssid":"BOSS-CONFIG","password":"123456789","port":5000,"uart":1,"bits":8,"baudrate":9600,"parity":"None","stop":1,"slave_address":1,"function_code":"0x03","start_address":20,"quantity":52,"api_url":"http://34.124.162.209/api-wash/","TX":16,"RX":17,"reload_time":1,"mqtt_status":"checked","request_status":"checked","broker_address": "34.124.162.209","broker_port":1883,"topic_subscribe":"washing_machine/data","topic_publish_status" :"washing_machine/status","mqtt_client_id" : "esp32","mqtt_user": "" ,"mqtt_password":""}
